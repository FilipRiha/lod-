var model = {
	...
	fire: function(guess) {
		for (var i = 0; i < this.numShips; i++) {
			...
			var index = ship.locations.indexOf(guess); // prohledá pole pro odhad a vrátí index
	...
	generateShip: function() {
		...
		var row, col;//řádek, sloupec

		if (direction === 1) {  //položí vodorovně
			row = Math.floor(Math.random() * this.boardSize);
			col = Math.floor(Math.random() * (this.boardSize - this.shipLength));
		} else { //položí vertikálně
